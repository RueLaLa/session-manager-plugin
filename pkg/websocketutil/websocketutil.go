...
func (u *WebsocketUtil) OpenConnection(url string, header http.Header) (*websocket.Conn, error) {
	...
	if header != nil {
		log.Infof("Opening websocket connection to: %s with auth header", url)
	} else {
	...
	if err != nil {
		if resp != nil {
			log.Errorf("Failed to dial websocket, status: %s, err: %s", resp.Status, err)
		} else {
			log.Errorf("Failed to dial websocket: %s", err)
		}
		return nil, err
	}
